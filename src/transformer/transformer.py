...
from torch.nn import Transformer, Module, ModuleList, Dropout, Linear, LayerNorm
...
class Transformer(Module):
    ...
    def __init__(self, d_model: int = 512, nhead: int = 8, num_encoder_layers: int = 6,
                 ...
                 device=None, dtype=None, max_pooling: int = None) -> None:
        ...
        if custom_encoder is not None:
            ...
            self.encoder = TransformerEncoder(encoder_layer, num_encoder_layers, encoder_norm, max_pooling=max_pooling)
...
class TransformerEncoder(Module):
    ...
    def __init__(self, encoder_layer, num_layers, norm=None, enable_nested_tensor=False, max_pooling: int = False):
    ...
    def forward(self, src: Tensor, mask: Optional[Tensor] = None, src_key_padding_mask: Optional[Tensor] = None) -> Tensor:
        ...
        for mod in self.layers:
            ...
            if self.max_pooling is not None:
                ...
                output = F.max_pool1d(output, self.max_pooling, ceil_mode=True)
                output = output.transpose(1, 2)
