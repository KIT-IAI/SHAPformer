...
class TimeSeriesTransformer(nn.Module):
    def __init__(self, d_model: int, input_features_count: int, num_encoder_layers: int, num_decoder_layers: int,
        ...
        self.transformer = Transformer(d_model, attention_heads, num_encoder_layers, num_decoder_layers,
                                       dim_feedforward, dropout, max_pooling=max_pooling, batch_first=True)
    ...
    def forward(self, x_enc, x_dec, src_mask=None, tgt_mask=None, memory_mask=None):
        """
        ...
        out = self.transformer(enc_embedding, dec_embedding, src_mask=src_mask, tgt_mask=tgt_mask, memory_mask=memory_mask)
    ...
    def get_decoder_self_attention_scores(self):
        return average_attention_scores([layer.self_attn.attention_weights
                                         for layer in self.transformer.decoder.layers])

    def get_encoder_self_attention_scores(self):
        return average_attention_scores([
            layer.self_attn.attention_weights for layer in self.transformer.encoder.layers
        ])
